...
export class SupportedPlatforms {
    static readonly DOTNET7 = new SupportedPlatforms('DOTNET7', {functionAppValue: "DOTNET|6.0", appServiceValue: "DOTNETCORE|7.0"});
    static readonly DOTNET6 = new SupportedPlatforms('DOTNET6', {functionAppValue: "DOTNET|6.0", appServiceValue: "DOTNETCORE|6.0"});
    static readonly NODE18 = new SupportedPlatforms('NODE18', {functionAppValue: "NODE|18-lts", appServiceValue: "NODE|18-lts"});
    static readonly NODE16 = new SupportedPlatforms('NODE16', {functionAppValue: "NODE|16-lts", appServiceValue: "NODE|16-lts"});
    static readonly PYTHON39 = new SupportedPlatforms('PYTHON39', {functionAppValue: "PYTHON|3.9", appServiceValue: "PYTHON|3.9"});
    static readonly PYTHON38 = new SupportedPlatforms('PYTHON38', {functionAppValue: "PYTHON|3.8", appServiceValue: "PYTHON|3.8"});
...
export abstract class AzureResource {
    ...
    constructor(apiVersion: string, name: string, tags: { [key: string]: string }[]) {
...
export class StorageAccount extends AzureResource {
    ...
    constructor(name: string, tags: { [key: string]: string }[], randomString?: string) {
        ...
        this.requiredProperties.map(property => { this.properties.push(property); });

        this.blobServices = new BlobServices(`[concat('${storageAccountName}', '/default')]`, tags, [`[resourceId('Microsoft.Storage/storageAccounts', '${storageAccountName}')]`]);
        this.subResources.push(this.blobServices);
        this.queueServices = new QueueServices(`[concat('${storageAccountName}', '/default')]`, tags, [`[resourceId('Microsoft.Storage/storageAccounts', '${storageAccountName}')]`]);
        this.subResources.push(this.queueServices);
        this.fileShareServices = new FileShareServices(`[concat('${storageAccountName}', '/default')]`, tags, [`[resourceId('Microsoft.Storage/storageAccounts', '${storageAccountName}')]`]);
        this.subResources.push(this.fileShareServices);
        this.tableServices = new TableServices(`[concat('${storageAccountName}', '/default')]`, tags, [`[resourceId('Microsoft.Storage/storageAccounts', '${storageAccountName}')]`]);
...
class BlobServices extends AzureResource {
    ...
    constructor(name: string, tags: { [key: string]: string }[], needs: string[]) {
        const apiVersion = "2022-05-01";
        super(apiVersion, name, tags);
        this.requiredProperties.map(property => { this.properties.push(property); });
        this.needs = needs;
        needs.map(need => { this.addDependsOn(need); });
    }
}
...
export class Application extends AzureResource {
    ...
    kind = "linux";
    sku: {
        name: "B1" | "B2" | "B3" | "S1" | "S2" | "S3" | "P1V2" | "P2V2" | "P3V2" | "P1V3" | "P2V3" | "P3V3"
    }
    ...
    constructor(type: AzureResourceType.FUNCTION_APP | AzureResourceType.APP_SERVICE, name: string, tags: { [key: string]: string }[], skuName: "B1" | "B2" | "B3" | "S1" | "S2" | "S3" | "P1V2" | "P2V2" | "P3V2" | "P1V3" | "P2V3" | "P3V3" = "B1", platform: SupportedPlatforms, appInsights: boolean = false) {
        ...
        this.storageAccount.subResources.forEach(resource => this.subResources.push(resource));
        ...
        this.site = new Site(uniqueName, tags, this.type, this.storageAccount.name, platform, siteDependsOn);
        this.subResources.push(this.site);
        this.hostNameBindings = new HostnameBindings(`[concat('${uniqueName}', '/', '${uniqueName}', '.azurewebsites.net')]`, tags, [`[resourceId('Microsoft.Web/sites', '${uniqueName}')]`]);
...
class ServerFarm extends AzureResource {
    ...
    constructor(name: string, tags: { [key: string]: string }[], skuName: "B1" | "B2" | "B3" | "S1" | "S2" | "S3" | "P1V2" | "P2V2" | "P3V2" | "P1V3" | "P2V3" | "P3V3" = "B1") {
        const apiVersion = "2022-03-01";
        super(apiVersion, name, tags);
        this.sku = {name: skuName};
        this.requiredProperties.map(property => { this.properties.push(property); });
...
class Site extends AzureResource {
    ...
    // Configurable property-value combos for this resource type
    serverFarmType: AzureResourceType.FUNCTION_APP | AzureResourceType.APP_SERVICE;
    platform: SupportedPlatforms;
    storageAccountName: string;
    // This resource type doesn't exist on its own, its dependencies must be included
    needs: string[];

    constructor(name: string, tags: { [key: string]: string }[], serverFarmType: AzureResourceType.FUNCTION_APP | AzureResourceType.APP_SERVICE, storageAccountName: string, platform: SupportedPlatforms, needs: string[]) {
        const apiVersion = "2022-03-01";
        super(apiVersion, name, tags);
        ...
        this.requiredProperties.map(property => { this.properties.push(property); });
        this.properties.push({ "name": this.name});
        this.properties.push({"serverFarmId": `[resourceId('Microsoft.Web/serverfarms', '${this.name}')]`});
        ...
        if (this.serverFarmType === AzureResourceType.APP_SERVICE) {
            this.kind = "app,linux";
            siteConfig["linuxFxVersion"] = `${this.platform.value["appServiceValue"]}`
            ...
            siteConfig["linuxFxVersion"] = `${this.platform.value["functionAppValue"]}`
            siteConfig["cors"] = {
                "allowedOrigins": [
                    "http://localhost:3000",
                    "http://localhost:3001",
        ...
        if (this.needs.includes(`[resourceId('Microsoft.Insights/components', '${this.name}')]`)) {
            siteConfig.appSettings.push({
...
export class PostgreSQL extends AzureResource {
    ...
    firewallRules?: { name: string, startIpAddress: string, endIpAddress: string }[];

    constructor(name: string, tags: { [key: string]: string }[], databaseNames?: string[], firewallRules?: { name: string, startIpAddress: string, endIpAddress: string }[]) {
        const apiVersion = "2022-01-20-preview";
        super(apiVersion, name, tags);
        this.requiredProperties.map(property => { this.properties.push(property); });
        this.properties.push({"AdministratorLoginPassword": this.generatePassword()});
        this.sasVPNFirewallRule = new PostgreSQLFirewallRule(`[concat('${this.name}', '/sas_vpn')]`, this.tags, "149.173.0.0", "149.173.255.255", [`[resourceId('Microsoft.DBforPostgreSQL/flexibleServers', '${this.name}')]`]);
        this.subResources.push(this.sasVPNFirewallRule);
        if (databaseNames) {
            databaseNames.forEach(databaseName => {
                let database = new PostgreSQLDatabase(`[concat('${this.name}', '/${databaseName}')]`, this.tags, [`[resourceId('Microsoft.DBforPostgreSQL/flexibleServers', '${this.name}')]`]);
                this.subResources.push(database);
            })
        }
        if (firewallRules) {
            firewallRules.forEach(rule => {
                let firewallRule = new PostgreSQLFirewallRule(`[concat('${this.name}', '/${rule.name}')]`, this.tags, rule.startIpAddress, rule.endIpAddress, [`[resourceId('Microsoft.DBforPostgreSQL/flexibleServers', '${this.name}')]`]);
...
class PostgreSQLFirewallRule extends AzureResource {
    ...
    constructor(name: string, tags: { [key: string]: string }[], startIpAddress: string, endIpAddress: string, needs: string[]) {
        const apiVersion = "2022-01-20-preview";
        super(apiVersion, name, tags);
