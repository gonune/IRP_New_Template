...
import { AzureResource, SupportedPlatforms, StorageAccount, Application, PostgreSQL } from "./resourceClasses";
...
export class ARMTemplate {
    ...
    createApplication(type: AzureResourceType.FUNCTION_APP | AzureResourceType.APP_SERVICE, name: string, platform: SupportedPlatforms, tags: { [key: string]: string }[] = [], skuName: "B1" | "B2" | "B3" | "S1" | "S2" | "S3" | "P1V2" | "P2V2" | "P3V2" | "P1V3" | "P2V3" | "P3V3" = "B1", appInsights: boolean = false) {
        // TODO: Error handling with try, catch
        let resource = this.arf.create(Application, type, name, tags, skuName, platform, appInsights);
    ...
    createPostgreSQL(name: string, tags: { [key: string]: string }[] = [], databaseNames?: string[], firewallRules?: { name: string, startIpAddress: string, endIpAddress: string }[]) {
        // TODO: Error handling with try, catch
        let resource = this.arf.create(PostgreSQL, name, tags, databaseNames, firewallRules);
        this.addResourceToCollection(resource);
    }
